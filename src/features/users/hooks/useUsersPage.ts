...
const useUsersPage = () => {
  ...
  // --- USERS LIST API/HOOK ---
  const { data, hasNextPage, hasPreviousPage, fetchNextPage, fetchPreviousPage, isLoading } =
    useUsersListHook(search);

  // flatten the paginated data
  const usersDataPage = data?.pages?.flatMap((page) => page?.data?.items);
